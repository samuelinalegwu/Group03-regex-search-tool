...
class RegexSearchApp:
    ...
    def _setup_styles(self):
        ...
        self.ACCENT_HOVER = "#6495ED"
        self.TEXT_COLOR = "#2c3e50"
        self.BORDER_COLOR = "#b0c4de"
        self.MATCH_HIGHLIGHT = "#fffacd"
        ...
        self.style.configure(".", background=self.BG_COLOR, foreground=self.TEXT_COLOR, font=self.default_font)
        ...
        self.style.configure("TLabelframe.Label", background=self.BG_COLOR, foreground=self.TEXT_COLOR, font=self.header_font)
        self.style.configure("TButton", padding=8, relief="flat", background=self.ACCENT_COLOR,
                             foreground=self.PRIMARY_COLOR, font=font.Font(family="Segoe UI", size=10, weight="bold"))
        self.style.map("TButton", background=[("active", self.ACCENT_HOVER), ("pressed", self.ACCENT_HOVER)])
        self.style.configure("TNotebook", background=self.BG_COLOR, borderwidth=0)
        self.style.configure("TNotebook.Tab", background=self.BG_COLOR, padding=[10, 5], font=self.default_font)
        ...
        self.style.configure("Treeview.Heading", font=self.header_font, background=self.BG_COLOR, relief="flat")
    ...
    def _create_regex_controls(self, parent):
        ...
        self.pattern_combo.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        ttk.Button(regex_frame, text="Save Query", command=self._save_query).pack(side=tk.LEFT)
        ...
        ttk.Checkbutton(flags_frame, text="Ignore Case", variable=self.ignore_case_var).pack(side=tk.LEFT, padx=15)
        ttk.Checkbutton(flags_frame, text="Multiline", variable=self.multiline_var).pack(side=tk.LEFT, padx=15)
        ttk.Checkbutton(flags_frame, text="Dot All", variable=self.dotall_var).pack(side=tk.LEFT, padx=15)
    ...
    def _create_file_search_tab(self):
        ...
        ttk.Button(file_controls, text="Browse File...", command=self._browse_file).pack(side=tk.LEFT)
        ttk.Button(file_controls, text="Search in File", command=self._perform_file_search).pack(side=tk.LEFT, padx=5)
        ...
        self.file_text_area.tag_configure("match", background=self.MATCH_HIGHLIGHT, foreground="black")

        bottom_frame = ttk.Frame(file_tab)
        bottom_frame.pack(fill=tk.X, pady=5)
        self.file_match_label = ttk.Label(bottom_frame, text="Matches Found: 0")
        self.file_match_label.pack(side=tk.LEFT)
        ttk.Button(bottom_frame, text="Download Matches", command=self._download_file_matches).pack(side=tk.RIGHT)
    ...
    def _create_folder_search_tab(self):
        ...
        ttk.Button(folder_controls, text="Browse Folder...", command=self._browse_folder).pack(side=tk.LEFT)
        ttk.Button(folder_controls, text="Search in Folder", command=self._perform_folder_search).pack(side=tk.LEFT, padx=5)

        results_frame = ttk.Labelframe(folder_tab, text="Search Results")
        ...
        self.context_text.tag_configure("match", background=self.MATCH_HIGHLIGHT, foreground="black")

        ttk.Button(folder_tab, text="Export Results to CSV", command=self._export_results).pack(pady=10)
    ...
    def _browse_file(self):
        filetypes = [
            ("All Supported Files", "*.txt *.py *.log *.md *.csv *.json *.pdf *.docx *.xlsx"),
    ...
    def _perform_file_search(self):
        ...
        if not pattern:
            messagebox.showwarning("Input Required", "Please enter a regex pattern.")
            return
        ...
        try:
            ...
            self.file_search_matches = find_matches_in_text(content, pattern, flags)
            self.file_match_label.config(text=f"Matches Found: {len(self.file_search_matches)}")
            ...
            messagebox.showerror("Invalid Regex", f"The regex pattern is invalid.\n\nDetails: {e}")

    def _download_file_matches(self):
        if not self.file_search_matches:
            messagebox.showwarning("No Matches", "There are no matches to download.")
            return
        ...
        if file_path:
            try:
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(f"--- Found {len(self.file_search_matches)} matches ---\n\n")
                    for match in self.file_search_matches:
                        f.write(match.group(0) + "\n")
                messagebox.showinfo("Download Complete", f"Matches saved to:\n{file_path}")
    ...
    def _perform_folder_search(self):
        ...
        if not folder or not pattern:
            messagebox.showwarning("Input Required", "Please select a folder and enter a regex pattern.")
            return
        ...
        try:
            flags = self._get_regex_flags()
            self.folder_search_results = list(self.searcher.search_in_folder(folder, pattern, flags))
    ...
    def _on_result_select(self, event):
        ...
        try:
            ...
            if start_index != -1:
                ...
                self.context_text.tag_add("match", f"1.{start_index}", f"1.{end_index}")
        except tk.TclError:
            pass
    ...
    def _export_results(self):
        if not self.folder_search_results:
            messagebox.showwarning("No Results", "There are no search results to export.")
            return
        ...
        if file_path:
            self.searcher.export_results_to_csv(self.folder_search_results, file_path)
    ...
    def _save_query(self):
        ...
        if query and query not in self.saved_queries:
            ...
            messagebox.showinfo("Query Saved", f"The pattern '{query}' has been saved.")
